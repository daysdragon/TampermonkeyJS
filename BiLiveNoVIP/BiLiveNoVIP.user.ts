...
class BiLiveNoVIP {
  ...
  private ChangeCSS() {
    ...
    //css内容
    let cssText = ''
    if (this.config.noVIPIcon.enable) cssText += '.vip-icon {display: none !important;}'
    if (this.config.noMedalIcon.enable) cssText += '.medal-icon {display: none !important;}'
    if (this.config.noUserLevelIcon.enable) cssText += '.user-level-icon {display: none !important;}'
    if (this.config.noLiveTitleIcon.enable) cssText += '.live-title-icon {display: none !important;}'
    if (this.config.noSystemMsg.enable) cssText += '.system-msg {display: none !important;} .announcement-container {display: none !important;}'
    if (this.config.noGiftMsg.enable) cssText += '.gift-msg {display: none !important;}'
    if (this.config.noSuperGift.enable) cssText += '.super-gift-ctnr {display: none !important;}'
    if (this.config.noSmallGift.enable) cssText += '.gift-msg-1000 {display: none !important;} .chat-msg-list {height: 100% !important;}'
    if (this.config.fixTreasure.enable) cssText += '.treasure {margin: -160px 0 !important;}'
...
@keyframes gunMeun {
  ...
  private PopularWords(disable: boolean) {
    ...
    this.XHR<Document>(player, 'document')
      .then((resolve) => {
