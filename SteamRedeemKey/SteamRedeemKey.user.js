...
let left;
let top;
if (GM_getValue('server') === undefined)
...
if (location.href.match(/^https:\/\/steamcn\.com\/.*289320/)) {
    const elmDivShowhid = document.querySelector('.showhide > p > a');
...
function showUI(event) {
    setTimeout(() => {
        ...
        if (str.length < 17)
            elmDivSRKButton.style.cssText = 'display: none;';
        else {
            const inputKeys = [];
            const elmInputKeys = selection.getRangeAt(0).cloneContents().querySelectorAll('input');
            for (const elmInputKey of elmInputKeys)
                inputKeys.push(elmInputKey.value);
            str += inputKeys.join(',');
...
function Options() {
    const elmDivOptions = document.querySelector('.showhide');
