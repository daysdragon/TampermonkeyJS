...
(function (name, factory) {
    ...
    hook.prototype.__fakeArgRst__ = function () {
        'use stric';
        if (typeof this.fn_puppet === 'function')
            return;
        var t = this;
     ...
     * 前后劫持全部关闭后，还原被 hook 的方法
	 ...
	 off('arg') 停止fakeArg的劫持
	 off('rst') 停止fakeRst的劫持
	 off() 停止所有劫持，还原被劫持的方法
     */
...
直接替换 .fakeArg(参数1, 参数2, ..) 原方法接收到的参数将以此为准
...
如果没有返回值会维持原来的参数不变
若要返回多个参数，使用数组 return [a, b, c]
返回空数组将清空所有参数值
.fakeRst()
修改原方法返回的参数

直接替换 .fakeRst(参数1) 众所周知js中只能返回一个值
